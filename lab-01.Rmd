...
NA means that some of the data is missing, so attempting to run a function on a NA data returns NA, because both true and false could be true or as in the example before any value could be the actual mean etc.
...
If it is set to true it excludes all NA data from the set before performing the function. 
...
There are more data bellow the mean but the difference between them and mean is smaller than the difference between mean and the high value data. 
